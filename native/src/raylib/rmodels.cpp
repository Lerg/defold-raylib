...
#include <math.h>           // Required for: sinf(), cosf(), sqrtf(), fabsf()
	...
	#include <raylib/external/tinyobj_loader_c.h>      // OBJ/MTL file formats loading
#endif
	...
	#include <raylib/external/cgltf.h>         // glTF file format loading
#endif
	...
	#include <raylib/external/vox_loader.h>    // VOX file format loading (MagikaVoxel)
#endif
	...
	#include <raylib/external/par_shapes.h>    // Shapes 3d parametric generation
#endif
...
void DrawMesh(Mesh mesh, Material material, Matrix transform)
{
	...
	for (int i = 0; i < MAX_MATERIAL_MAPS; i++)
	{
		...
		if (i == MATERIAL_MAP_NORMAL) {
			rlSetUniform(material.shader.locs[SHADER_LOC_USE_MAP_NORMAL], &use, SHADER_UNIFORM_INT, 1);
		} else if (i == MATERIAL_MAP_METALNESS) {
			rlSetUniform(material.shader.locs[SHADER_LOC_USE_MAP_METALLNESS], &use, SHADER_UNIFORM_INT, 1);
		}
	}
...
Material *LoadMaterials(const char *fileName, int *materialCount)
{
	...
	if (IsFileExtension(fileName, ".mtl"))
	{
		...
		int result = tinyobj_parse_mtl_file_mem(&mats, &count, file_content, strlen(file_content));
...
void DrawModel(Model model, Vector3 position, float scale, Color tint)
{
	...
	DrawModelEx(model, position, QuaternionIdentity(), vScale, tint);
}

// Draw a model with extended parameters
void DrawModelEx(Model model, Vector3 position, Quaternion rotation, Vector3 scale, Color tint)
{
...
void DrawModelWiresEx(Model model, Vector3 position, Quaternion rotation, Vector3 scale, Color tint)
{
	rlEnableWireMode();

	DrawModelEx(model, position, rotation, scale, tint);
...
static Model LoadOBJ(const char *fileName)
{
	...
	char *mtlFileName = (char *)RL_MALLOC(fileNameLen);
	strncpy(mtlFileName, fileName, fileNameLen);
	mtlFileName[fileNameLen] = '\0';
	mtlFileName[fileNameLen - 3] = 'm';
	mtlFileName[fileNameLen - 2] = 't';
	mtlFileName[fileNameLen - 1] = 'l';
	char *mtlFileText = NULL;
	...
	if (fileText != NULL)
	{
		...
		mtlFileText = LoadFileText(mtlFileName);
		unsigned int mtlDataSize = 0;
		if (mtlFileText != NULL) {
			mtlDataSize = (unsigned int)strlen(mtlFileText);
		}

		unsigned int flags = TINYOBJ_FLAG_TRIANGULATE;
		int ret = tinyobj_parse_obj_mem(&attrib, &meshes, &meshCount, &materials, &materialCount, fileText, dataSize, flags, mtlFileText, mtlDataSize);
			...
			for (unsigned int p = 0; p < data->meshes[i].primitives_count; p++)
			{
				...
				for (unsigned int j = 0; j < data->meshes[i].primitives[p].attributes_count; j++)
				{
					...
					else if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_texcoord) // TEXCOORD_0
					{
						...
						if ((attribute->component_type == cgltf_component_type_r_32f) && (attribute->type == cgltf_type_vec2))
						{
							...
							model.meshes[meshIndex].texcoords = (float *)RL_MALLOC(attribute->count*2*sizeof(float));
				...
				if (data->meshes[i].primitives[p].indices != NULL)
				{
					...
					else if (attribute->component_type == cgltf_component_type_r_32u)
					{
						...
						// Load data into a temp buffer to be converted to raylib data type
						unsigned int *temp = (unsigned int *)RL_MALLOC(attribute->count*sizeof(unsigned int));
